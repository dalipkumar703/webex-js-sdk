...
const KMS = WebexPlugin.extend({
  ...
  updateKmsKey(activity) {
    /* detach activity key */
    const detachKeyParams = {
      uri: 'https://conversation-intb.ciscospark.com/conversation/api/v1/conversations/67acfbf0-35c9-11ea-ad4b-05dbb1640965/healed?resetKey=true'
    };

    return this.request(detachKeyParams).then((res) => {
      console.log(res);

      /* call create unbound key */
      console.log('calling unbound key');
      this.createUnboundKeys({count: 1}).then((unboundKeyResponse) => {
        console.log('resolved');
        this.bindKey({
          kro: null, kroUri: activity.target.kmsResourceObjectUrl, key: null, keyUri: unboundKeyResponse[0].uri
        }).then((res) => {
          this.unboundedStorage.put(unboundKeyResponse[0].uri, JSON.stringify(res, this.webex.internal.encryption.replacer));
          //  /* update key */
          //   this.webex.internal.conversation.get({url: activity.target.url}).then((conversation)=>{
          //     this.webex.internal.conversation.updateKey(conversation, unboundKeyResponse[0].uri, activity).then(()=>{console.log("activated key")});

          //   })
          //   console.log('resolved bindKey');
        });
      }).catch((error) => {
        console.log('error in key activation', error);
      });
    });
  }
});
