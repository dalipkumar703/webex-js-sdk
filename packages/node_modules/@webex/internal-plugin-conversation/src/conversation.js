...
import ConversationConstant from './config';
...
const Conversation = WebexPlugin.extend({
  ...
  */
  ...
  submit(activity, endpoint) {
    ...
    if (activity.target && !activity.target.defaultActivityEncryptionKeyUrl) {
      const updateKeyActivity = {
        // Needed to display in the stream instantly
        actor: {
          objectType: ConversationConstant.conversation.updateKeyActivity.PERSON,
          id: activity.actor.id
        },
        // Needed for round trip
        // clientTempId: API.TEMP_ID_PREFIX + uuid.v4(),
        // Minimum properties needed by API
        object: {
          displayName: ConversationConstant.conversation.updateKeyActivity.UPDATE_KEY,
          content: ConversationConstant.conversation.updateKeyActivity.UPDATE_KEY,
          objectType: ConversationConstant.conversation.updateKeyActivity.CONVERSATION
        },
        target: {
          id: activity.target.id, // accept convo object or ID
          objectType: ConversationConstant.conversation.updateKeyActivity.CONVERSATION,
          url: activity.target.url
        },
        verb: ConversationConstant.conversation.updateKeyActivity.UPDATE_KEY,
        published: new Date().toISOString(),
        clientPublished: new Date().toISOString(), // use to bundle correctly in the activitylist
        _status: 'PENDING'
      };

      return Promise.resolve(this.webex.internal.encryption.kms.updateKmsKey(updateKeyActivity)).then((updateKmsResponse) => {
        activity.target.defaultActivityEncryptionKeyUrl = updateKmsResponse.target.defaultActivityEncryptionKeyUrl;

        return this.submit(activity);
      });
    }
  ...
  _updateKey(conversation, key, activity) {
    ...
    return Promise.resolve(
      ...
      .then((keys) => {
        ...
        /* update key in case of empty */

        if (activity.target && !activity.target.defaultActivityEncryptionKeyUrl) {
          activity.target.defaultActivityEncryptionKeyUrl = k.uri;
          convoWithUrl.defaultActivityEncryptionKeyUrl = k.uri;
        }
        const params = {
          verb: 'updateKey',
          target: this.prepareConversation(convoWithUrl),
